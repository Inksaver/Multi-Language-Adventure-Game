...
def process_input(prompt:str, min_value:int, max_value:int, data_type:str, row, window_width:int = 0) -> str:
	...
	while valid_input is False:
		if window_width > 0:
			clear_input_field(row + 2)
			clear_input_field(row + 1)
			clear_input_field(row)
		print("".ljust(width, "─"))
		if window_width > 0:
			print()
			print("".ljust(window_width, "─"))
...
def get_string(prompt:str, with_title:bool = False, min_value:int = 1, max_value:int = 20, row:int = -1, window_width:int = 0) -> str: # with_title, min_value and max_value can be over-ridden by calling code
	...
	if row >= 0:
		row += 1
	result = process_input(prompt, min_value, max_value, "string", row, window_width)
	if with_title:
		result = result.title()
	return result
	
def get_integer(prompt:str, min_value:int = 0, max_value:int = 65536, row:int = -1, window_width:int = 0) -> int: # min_value and max_value can be over-ridden by calling code
	''' Public Method: gets an integer from the user '''
	if row >= 0:
		row += 1	
	return process_input(prompt, min_value, max_value, "int", row, window_width)

def get_float(prompt:str, min_value:float = 0.0, max_value:float = 1000000.0, row:int = -1, window_width:int = 0) -> float: # min_value and max_value can be over-ridden by calling code
	''' Public Method: gets a float from the user '''
	if row >= 0:
		row += 1	
	return process_input(prompt, min_value, max_value, "float", row, window_width)
	
def get_boolean(prompt:str, row:int = -1, window_width:int = 0) -> bool:
	''' Public Method: gets a boolean (yes/no) type entries from the user '''
	if row >= 0:
		row += 1	
	return process_input(prompt, 1, 3, "bool", row, window_width)
...
def menu(title:str, menu_list:list, row = -1, window_width = 0) -> int:
	...
	max_len = get_max_len(title, menu_list, width)
	...
	return get_integer(f"Type the number of your choice (1 to {len(menu_list)})", 1, len(menu_list), rows, window_width) - 1 # -1 to return correct list index
