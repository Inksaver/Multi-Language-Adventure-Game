...
namespace Adventure_06_Improvements
{
    internal class Kboard
    {
        ...
        public static int Clear()
        {
            ...
            Console.Clear();
            return 0;   // row 0 on console assumed
        }
        ...
        public static int GetInteger(string prompt, double min, double max, int row = -1, int windowWidth = 0)
        {
            ...
            string userInput = ProcessInput(prompt, min, max, dataType:"int", row, windowWidth);
        ...
        public static int Menu(string title, List<string> textLines, int row = -1, int windowWidth = 0)
        {
            ...
            for (int i = 0; i < textLines.Count; i++)
            {
                ...
                else        Print($"║    {i + 1}) {textLines[i].PadRight(maxLength - 8)}║");   // print menu options 4 spaces
            }
            Print($"╚{new string('═', maxLength)}╝");
