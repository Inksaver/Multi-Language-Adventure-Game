    ...
    internal static class Game
    {
            ...
            else
            {
                if (Shared.Debug)
                {
                    ...
                    foreach (KeyValuePair<string, Location> kvp in Shared.Locations)
                    {
                        ...
                        Console.Write($"Item required: {kvp.Value.ItemRequired.PadRight(12)}");
                        string itemList = "│ Items: ";
                        foreach (string item in kvp.Value.Items) // empty list or list of string key values
                            itemList += $"{item}, ";
                        Console.Write($"{itemList.PadRight(49)}Enemy: {kvp.Value.Enemy}");
                        Console.Write("\n");
                        Console.WriteLine(new string('─', Console.WindowWidth - 1));
                    }
        ...
        private static void CreateDefaultEnemies()
        {
            ...
            if(Shared.Debug)
            {
                ...
                {
                    Console.WriteLine($"Name:     {kvp.Key.PadRight(10)}Description: {kvp.Value.Description}");

                    Console.WriteLine($"Strength: {kvp.Value.Strength.ToString().PadRight(10)}" +
                                      $"Health: {kvp.Value.Health.ToString().PadRight(8)}" +
                                      $"Drop Item: {kvp.Value.DropItem}");
                }
                Console.WriteLine(new string('═', Console.WindowWidth - 1));
                Console.Write("Enter to continue");
                Console.ReadLine();
        ...
        private static void CreateDefaultItems()
        {
            ...
            */
            ...
            AddToWeapons("sword", "a toy plastic sword: a dog has chewed the handle..Yuk!", new List<string>() , 0, "", damage:25);
